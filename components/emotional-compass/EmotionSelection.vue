<template>
  <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
    <button v-for="emotion in emotions" :key="emotion.id" @click="$emit('select-emotion', emotion)"
      class="group relative p-4 rounded-2xl transition-all duration-300 border hover:-translate-y-1" :class="[
      ...
      <div class="relative text-center">
        <span class="block text-sm font-bold tracking-wide transition-colors duration-300" :class="[
          selectedEmotion?.id === emotion.id
