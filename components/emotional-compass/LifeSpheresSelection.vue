<template>
  <div class="flex flex-wrap justify-center gap-3 max-w-3xl mx-auto">
    <button v-for="sphere in lifeSpheres" :key="sphere.name" @click="$emit('toggle-tag', sphere.name)"
      ...
      <!-- Active Background -->
      <div v-if="selectedTags.includes(sphere.name)" class="absolute inset-0 opacity-20 transition-opacity duration-300"
        :class="sphere.color"></div>
