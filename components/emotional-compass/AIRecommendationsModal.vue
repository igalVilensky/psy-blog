<template>
  <div v-if="isOpen" class="fixed inset-0 z-[100] flex items-start justify-center p-4 pt-24 overflow-y-auto">
    <!-- Backdrop -->
    ...
    <div
      ...
      <div class="px-6 pb-6 min-h-[300px] flex flex-col">
        ...
        <div v-if="isGuest" class="flex-1 flex flex-col items-center justify-center text-center space-y-6">
          <div
            class="w-20 h-20 rounded-full bg-cyan-100 dark:bg-[#0EA5E9]/10 flex items-center justify-center mb-4 border border-cyan-500/30">
          ...
          <p class="text-slate-600 dark:text-slate-300 max-w-md">
            Хотите получить персональные рекомендации от ИИ на основе ваших записей? Войдите или создайте аккаунт, чтобы
            разблокировать эту функцию.
          </p>
          <div class="flex gap-4 mt-4">
            <Button text="Войти" iconClass="fas fa-sign-in-alt" gradientStart="#8B5CF6" gradientEnd="#06B6D4"
              textColor="#FFFFFF"
              ...
              :isLink="true" to="/login" />
            <Button text="Регистрация" iconClass="fas fa-user-plus" gradientStart="#8B5CF6" gradientEnd="#06B6D4"
              textColor="#FFFFFF"
              customClass="relative inline-flex items-center justify-center px-6 py-2 overflow-hidden font-medium transition-all duration-300 ease-out rounded-lg group border-[#0EA5E9]"
              :isLink="true" to="/register" />
          </div>
        </div>
        ...
        <div v-else-if="status === 'idle'"
          ...
          <div
            class="w-20 h-20 rounded-full bg-cyan-100 dark:bg-[#0EA5E9]/20 flex items-center justify-center animate-pulse">
            <i class="fas fa-magic text-3xl text-cyan-600 dark:text-[#0EA5E9]"></i>
          </div>
          <p class="text-slate-600 dark:text-slate-300 max-w-md">
            На основе ваших записей мы можем сгенерировать персональные советы и практики для улучшения вашего
            состояния.
          </p>
          <Button text="Получить рекомендации" iconClass="fas fa-sparkles" gradientStart="#8B5CF6" gradientEnd="#06B6D4"
            textColor="#0EA5E9"
            ...
            :isLink="false" @click="getRecommendations" />
        </div>
        ...
        <div v-else-if="status === 'loading'"
          ...
          <div class="relative w-24 h-24">
            ...
            <div class="absolute inset-2 rounded-full border-r-2 border-[#E879F9] animate-spin animation-delay-150">
            </div>
            <div class="absolute inset-4 rounded-full border-b-2 border-[#8B5CF6] animate-spin animation-delay-300">
            </div>
          </div>
        ...
        <div v-else-if="status === 'success'" class="space-y-6">
          ...
          <div class="prose prose-invert max-w-none">
            <div
              class="text-slate-700 dark:text-slate-300 whitespace-pre-line leading-relaxed bg-slate-50 dark:bg-[#1A1F35]/40 p-4 rounded-xl border border-cyan-500/10">
              {{ recommendations }}
            </div>
          </div>
        </div>
        ...
        <div v-else-if="status === 'error'"
          ...
          <Button text="Попробовать снова" iconClass="fas fa-redo" gradientStart="#EF4444" gradientEnd="#F87171"
            textColor="#FFFFFF"
      ...
      <div class="p-6 flex justify-end gap-3 border-t border-cyan-500/20 bg-slate-50 dark:bg-[#1A1F35]/80">
        <Button text="Закрыть" iconClass="fas fa-times" textColor="#CBD5E1"
