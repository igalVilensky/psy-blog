  ...
  <div class="space-y-6">
    <div class="flex flex-row items-center gap-4 mb-4">
      <div
        ...
        <i class="fas fa-fingerprint text-stone-900 dark:text-white text-xl"></i>
      </div>
      <div>
        <h2 class="text-xl sm:text-2xl font-bold uppercase tracking-tight text-stone-900 dark:text-white">
          Идентичность</h2>
        <p class="text-xs font-bold text-stone-500 uppercase tracking-wide mt-1">Stuctura Personalis</p>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
      <!-- Personality Radar -->
      <div
        class="bg-white dark:bg-stone-900 p-6 border border-stone-200 dark:border-stone-800 border-l-4 border-l-purple-500">
        <div class="flex items-center justify-between mb-6">
          <h3 class="font-bold uppercase tracking-wide text-stone-900 dark:text-white">Профиль (Big Five)</h3>
          <NuxtLink to="/space/tests/big-5-model"
            class="text-xs font-bold uppercase tracking-wider text-purple-600 hover:text-purple-700">Перепройти
          </NuxtLink>
        </div>
        ...
        <div v-if="dominantInsight" class="mb-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div
            class="p-4 bg-purple-50 dark:bg-purple-900/10 border border-purple-100 dark:border-purple-800/20 border-l-4 border-l-purple-400">
            <div class="text-[10px] text-purple-600 dark:text-purple-400 font-bold uppercase tracking-wider mb-1">
              Сильная сторона</div>
            <div class="text-sm font-bold text-stone-900 dark:text-white mb-1 uppercase">{{
              dominantInsight.strength.trait
              }}</div>
            <div class="text-[10px] text-stone-500 font-medium leading-tight">{{ dominantInsight.strength.text }}</div>
          </div>
          <div
            class="p-4 bg-stone-50 dark:bg-stone-800 border border-stone-200 dark:border-stone-700 border-l-4 border-l-stone-400">
            <div class="text-[10px] text-stone-500 font-bold uppercase tracking-wider mb-1">Зона роста</div>
            <div class="text-sm font-bold text-stone-900 dark:text-white mb-1 uppercase">{{ dominantInsight.growth.trait
              }}
            </div>
            <div class="text-[10px] text-stone-500 font-medium leading-tight">{{ dominantInsight.growth.text }}</div>
          </div>
        </div>
        ...
        <div class="relative h-72 flex items-center justify-center">
          <svg viewBox="0 0 500 500" class="w-full h-full max-w-[360px]">
            ...
            <circle v-for="i in 5" :key="`circle-${i}`" cx="250" cy="250" :r="i * 32" fill="none" stroke="currentColor"
              class="stroke-stone-200 dark:stroke-stone-700" stroke-width="1" />

            <!-- Links for axes -->
            <line v-for="i in 5" :key="`line-${i}`" x1="250" y1="250" :x2="getDataPoint(i - 1, 100).x"
              :y2="getDataPoint(i - 1, 100).y" stroke="currentColor" class="stroke-stone-200 dark:stroke-stone-700"
              stroke-width="1" />
            ...
            <g v-for="(point, idx) in radarDataPoints" :key="`dot-group-${idx}`" class="group cursor-pointer">
              <circle :cx="point.x" :cy="point.y" r="4"
                class="fill-white dark:fill-stone-900 stroke-purple-600 dark:stroke-purple-400 transition-all duration-300 group-hover:r-6"
                stroke-width="2" />
            </g>
            ...
            <g v-for="(trait, index) in traits" :key="`label-${trait.name}`">
              <rect :x="getLabelPosition(index).x - 60" :y="getLabelPosition(index).y - 14" width="120" height="28"
                class="fill-white/80 dark:fill-stone-900/80" />
              <text :x="getLabelPosition(index).x" :y="getLabelPosition(index).y + 5" text-anchor="middle"
                class="text-[12px] font-bold fill-stone-900 dark:fill-white uppercase tracking-wider pointer-events-none">
                {{ trait.name }}
              </text>
            </g>
          </svg>
      ...
      <div class="flex flex-col gap-6">
        <!-- Archetypes Summary -->
        <div
          class="bg-stone-50 dark:bg-stone-800/30 p-6 border border-stone-200 dark:border-stone-700 border-l-4 border-l-amber-500">
          <h3 class="font-bold uppercase tracking-wide text-stone-900 dark:text-white mb-4 flex items-center gap-2">
            <i class="fas fa-masks-theater text-amber-600"></i>
            Ваши Архетипы
          </h3>
          <div class="flex flex-wrap gap-2">
            <div v-for="arc in topArchetypes" :key="arc.name"
              class="px-3 py-1 bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-700 text-xs font-bold uppercase tracking-wider text-stone-700 dark:text-stone-300 flex items-center gap-2">
              <span>{{ arc.name }}</span>
              <span class="text-amber-600 dark:text-amber-500">{{ arc.level }}%</span>
            </div>
            <div v-if="!topArchetypes.length" class="text-xs font-bold text-stone-400 uppercase tracking-wider">Тест на
        ...
        <!-- Recent Achievements -->
        <div
          class="bg-white dark:bg-stone-900 p-6 border border-stone-200 dark:border-stone-800 border-l-4 border-l-emerald-500 shadow-sm flex-1">
          <h3 class="font-bold uppercase tracking-wide text-stone-900 dark:text-white mb-4">Вехи роста</h3>
          <div class="space-y-4">
            <div v-for="ach in achievements.slice(0, 3)" :key="ach.id" class="flex items-start gap-4 group">
              <div
                class="w-8 h-8 flex items-center justify-center bg-stone-100 dark:bg-stone-800 border border-stone-200 dark:border-stone-700">
                <i :class="[ach.icon, 'text-sm']"></i>
              </div>
              <div class="flex-1">
                <div class="text-xs font-bold uppercase tracking-wide text-stone-900 dark:text-white mb-0.5">{{ ach.name
                  }}</div>
                <div class="text-[10px] text-stone-500 font-medium uppercase tracking-wider">{{ ach.description }}</div>
              </div>
            </div>
            <div v-if="!achievements.length"
              class="text-center py-4 text-stone-400 text-xs font-bold uppercase tracking-wider">Начните тренировки
            </div>
          </div>
        </div>
      </div>
