      ...
      <li v-for="post in posts" :key="post._id" class="group">
        <a :href="`/blog/${post.slug?.current}`" class="block">
          <div class="flex flex-col space-y-4">
            <!-- Featured image -->
            <div
              class="rounded-lg overflow-hidden bg-gray-100 aspect-video relative"
            >
              <nuxt-img
                v-if="post.image"
            ...
            <div>
              <h3
                class="text-xl font-bold text-white group-hover:text-indigo-600 transition-colors"
              >
                {{ post.title }}
              </h3>
              ...
              <div class="mt-4 flex items-center text-sm text-gray-500">
                <span v-if="post.author?.name" class="flex items-center">
                  <div
                    class="w-6 h-6 rounded-full bg-gray-200 overflow-hidden mr-2"
                  >
                    <img
                      v-if="post.author.image?.asset?.url"
