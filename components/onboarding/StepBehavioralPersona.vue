<template>
  <div class="space-y-8">
    ...
    <div class="space-y-6">
      <!-- Pair 1: Thinking -->
      <div class="space-y-3">
        <h3 class="text-sm font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider text-center">Мышление
        </h3>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <button v-for="option in pairs.thinking" :key="option" @click="selectOption('thinking', option)"
            class="p-4 rounded-xl border-2 text-center transition-all duration-200" :class="[
      ...
      <div class="space-y-3">
        <h3 class="text-sm font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider text-center">Эмоции
        </h3>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <button v-for="option in pairs.processing" :key="option" @click="selectOption('processing', option)"
            class="p-4 rounded-xl border-2 text-center transition-all duration-200" :class="[
      ...
      <div class="space-y-3">
        <h3 class="text-sm font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider text-center">
          Структура</h3>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <button v-for="option in pairs.structure" :key="option" @click="selectOption('structure', option)"
            class="p-4 rounded-xl border-2 text-center transition-all duration-200" :class="[
      ...
      <div class="space-y-3">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <button v-for="option in pairs.tone" :key="option" @click="selectOption('tone', option)"
            class="p-4 rounded-xl border-2 text-center transition-all duration-200" :class="[
    ...
    <div class="flex justify-between pt-6">
      ...
      <button @click="$emit('next')" :disabled="!isComplete"
