<template>
  ...
  >
    <div class="flex items-center gap-3 sm:gap-4 mb-3">
      ...
      >
        <i :class="iconClass" class="text-[#0EA5E9] text-sm sm:text-base"></i>
      </div>
      <p class="text-slate-400 font-medium text-sm sm:text-base">{{ title }}</p>
    </div>
    <!-- Regular Value -->
    <p
      v-if="!isSocialMedia && value"
      class="text-white/90 text-base sm:text-lg"
    >
      {{ value }}
    </p>
    <!-- Social Media Links -->
    <div
      v-else-if="isSocialMedia && Array.isArray(value) && value.length > 0"
      class="space-y-2"
    >
      <div
        v-for="(platform, index) in value"
        :key="index"
      ...
      >
        ...
        ></i>
        <a
          :href="normalizeUrl(platform.type, platform.url)"
          target="_blank"
