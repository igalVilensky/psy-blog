<template>
  <div class="w-full max-w-6xl mb-12 relative">
    ...
    <div class="absolute inset-0 z-0 hidden md:block">
      ...
      <div class="absolute top-[50%] left-0 right-0 h-0.5 bg-gradient-to-r from-[#0EA5E9]/30 to-[#E879F9]/30"></div>
      <div class="absolute top-[85%] left-0 right-0 h-0.5 bg-gradient-to-r from-[#0EA5E9]/30 to-[#E879F9]/30"></div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
      ...
      <div class="md:col-start-2 md:col-span-1">
        <div class="flex flex-col md:flex-row">
          <SphereCard title="Сущность" subtitle="Адам Кадмон" icon-class="fas fa-crown"
            description="Ваша истинная природа, основа всего, потенциал души." gradient-start="#7B68EE"
            gradient-end="#E879F9" icon-color="#7B68EE" :loading="loadingBio" :data="bioData"
            button-text="Редактировать" button-to="/profile/edit" button-icon="fas fa-pen"
            :progress="calculateEssenceProgress()" progress-label="Завершённость профиля">
            <template #data-content>
              <div class="flex-shrink-0 flex justify-center items-center mb-4">
                <UserAvatar :avatar-url="avatarUrl" :loading="loadingBio" :user-initial="userInitial" :size="100"
                  :no-upload="true" />
              </div>
      ...
      <div class="md:col-start-1 md:col-span-1">
        <SphereCard title="Прозрение" subtitle="Ацилут" icon-class="fas fa-lightbulb"
          description="Интеллектуальное понимание, сфера мудрости и осознания." gradient-start="#0EA5E9"
          gradient-end="#9333EA" icon-color="#0EA5E9" :loading="loadingAssessments" :data="insightData"
          button-text="Исследовать архетипы" button-to="/courses" button-icon="fas fa-arrow-right"
          :progress="Math.min(insightData.archetypeScores.length * 20, 100)" progress-label="Прогресс прозрения">
          <template #data-content>
            <div class="flex justify-between items-center">
      ...
      <div class="md:col-start-3 md:col-span-1">
        <SphereCard title="Действие" subtitle="Асия" icon-class="fas fa-running"
          description="Материальный план, поведение и конкретные действия." gradient-start="#F59E0B"
          gradient-end="#EF4444" icon-color="#F59E0B" :loading="loadingEmotionBarometer" :data="actionData"
          button-text="Все инструменты" button-to="/awareness-tools" button-icon="fas fa-tools" :progress="Math.min(
            actionData.emotionBarometerStats.totalEntries * 10 +
            actionData.archetypeScores.length * 10,
            100
          )
            " progress-label="Активность действий">
          <template #data-content v-if="actionData.emotionBarometerStats.totalEntries > 0 ||
          ...
          <template #no-data>
            <i class="fas fa-shoe-prints text-[#F59E0B] text-2xl opacity-50"></i>
            <p class="text-center text-slate-400">Нет данных о поведении</p>
            <NuxtLink to="/awareness-tools" class="text-[#F59E0B] hover:underline text-sm">
