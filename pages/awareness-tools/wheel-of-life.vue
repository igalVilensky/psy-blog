  ...
  <div class="min-h-screen text-slate-200">
    <div class="container mx-auto px-4 py-8 max-w-7xl">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-7 space-y-8">
          ...
          <div class="bg-slate-900/40 backdrop-blur-xl border border-slate-800/60 rounded-2xl p-6 shadow-2xl relative overflow-hidden group">
             <!-- Decorative background elements -->
            <div class="absolute top-0 right-0 w-64 h-64 bg-cyan-500/5 rounded-full blur-3xl -z-10"></div>
            <div class="absolute bottom-0 left-0 w-64 h-64 bg-purple-500/5 rounded-full blur-3xl -z-10"></div>
            ...
            <div class="relative flex justify-center items-center min-h-[300px] md:min-h-[400px]">
               ...
               <div class="absolute inset-0 flex items-center justify-center pointer-events-none">
                 <div class="bg-slate-900/80 backdrop-blur-md rounded-full p-3 border border-slate-700/50 shadow-lg">
                   <span class="text-xs font-mono text-cyan-400">БАЛАНС</span>
                 </div>
               </div>
            </div>
          </div>
          ...
          <div class="bg-slate-900/40 backdrop-blur-xl border border-slate-800/60 rounded-2xl p-6 shadow-xl">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div v-for="(category, index) in categories" :key="index" class="group">
                <div class="flex justify-between items-center mb-2">
                  <label :for="`cat-${index}`" class="text-sm font-medium text-slate-300 group-hover:text-cyan-300 transition-colors flex items-center gap-2">
                    <span class="w-2 h-2 rounded-full" :style="{ backgroundColor: category.color }"></span>
                    {{ category.name }}
                  </label>
                  <span class="text-xs font-mono bg-slate-800 px-2 py-1 rounded text-cyan-400 border border-slate-700">
                    {{ category.score }}/10
                  </span>
                </div>
                <input
                  ...
                  min="1"
                  max="10"
                  class="w-full h-2 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-cyan-500 hover:accent-cyan-400 transition-all"
                  @input="updateWheel"
                />
              </div>
            </div>
        ...
        <div class="lg:col-span-5 space-y-6">
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            >
              ...
              <span class="text-sm font-medium text-cyan-100">Сохранить</span>
            </button>
            <button 
            ...
            >
              ...
              <span class="text-sm font-medium text-purple-100">Цели</span>
            </button>
          </div>
          ...
          <div class="bg-slate-900/40 backdrop-blur-xl border border-slate-800/60 rounded-2xl p-6 shadow-xl">
            ...
            <div v-if="lowScoreCategories.length > 0" class="space-y-4">
              <div v-for="(cat, idx) in lowScoreCategories.slice(0, 2)" :key="idx" 
                   class="bg-slate-800/50 rounded-lg p-4 border-l-4 border-red-500/70">
                <h4 class="font-medium text-slate-200 mb-1">{{ cat.name }} ({{ cat.score }}/10)</h4>
                <p class="text-sm text-slate-400">{{ getRecommendationForCategory(cat.name) }}</p>
              </div>
            </div>
           ...
           <div class="bg-slate-900/40 backdrop-blur-xl border border-slate-800/60 rounded-2xl p-6 shadow-xl">
             <div class="flex justify-between items-center mb-4">
                ...
                <button @click="showEnergyModal = true" class="text-xs text-cyan-400 hover:text-cyan-300 underline">Настроить</button>
             </div>
             <div class="flex gap-2 overflow-x-auto pb-2">
                ...
                <span v-if="!energizingCategories.length && !drainingCategories.length" class="text-sm text-slate-500 italic">Нет данных</span>
             </div>
           </div>
        </div>
      </div>
    ...
    <Transition name="modal">
      <div v-if="showGoalModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
        <div class="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md p-6 shadow-2xl relative">
          <button @click="showGoalModal = false" class="absolute top-4 right-4 text-slate-400 hover:text-white">
            <i class="fas fa-times"></i>
          </button>
          <h3 class="text-xl font-bold text-white mb-4">Новая цель</h3>
          
          <div class="space-y-4">
            <div>
              <label class="block text-sm text-slate-400 mb-1">Сфера</label>
              <select v-model="newGoal.category" class="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-white focus:border-cyan-500 outline-none">
                <option v-for="cat in categories" :value="cat.name">{{ cat.name }}</option>
              </select>
            </div>
            <div>
               <label class="block text-sm text-slate-400 mb-1">Цель (1-10)</label>
               <input v-model.number="newGoal.targetScore" type="range" min="1" max="10" class="w-full accent-cyan-500">
               <div class="text-right text-cyan-400 font-mono">{{ newGoal.targetScore }}</div>
            </div>
            <div>
              <label class="block text-sm text-slate-400 mb-1">Описание</label>
              <textarea v-model="newGoal.description" rows="3" class="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-white focus:border-cyan-500 outline-none"></textarea>
            </div>
          </div>

          <div class="mt-6 flex justify-end gap-3">
            <button @click="showGoalModal = false" class="px-4 py-2 text-slate-400 hover:text-white">Отмена</button>
    ...
    <Transition name="modal">
      <div v-if="showEnergyModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
        <div class="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md p-6 shadow-2xl relative max-h-[80vh] overflow-y-auto">
          <button @click="showEnergyModal = false" class="absolute top-4 right-4 text-slate-400 hover:text-white">
            <i class="fas fa-times"></i>
          </button>
          <h3 class="text-xl font-bold text-white mb-4">Источники энергии</h3>
          
          <div class="space-y-3">
            <div v-for="(cat, idx) in categories" :key="idx" class="bg-slate-800/50 p-3 rounded-lg border border-slate-700/50">
              <div class="flex justify-between items-center mb-2">
                <span class="font-medium text-slate-200">{{ cat.name }}</span>
              </div>
              <div class="flex gap-2">
                <button @click="cat.energy = 'energizing'" 
                  :class="['flex-1 py-1 rounded text-xs font-medium transition-colors', cat.energy === 'energizing' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/50' : 'bg-slate-700 text-slate-400 hover:bg-slate-600']">
                  + Энергия
                </button>
                <button @click="cat.energy = 'neutral'"
                   :class="['flex-1 py-1 rounded text-xs font-medium transition-colors', cat.energy === 'neutral' ? 'bg-slate-600 text-white' : 'bg-slate-700 text-slate-400 hover:bg-slate-600']">
                  Нейтрально
                </button>
                <button @click="cat.energy = 'draining'"
                   :class="['flex-1 py-1 rounded text-xs font-medium transition-colors', cat.energy === 'draining' ? 'bg-red-500/20 text-red-400 border border-red-500/50' : 'bg-slate-700 text-slate-400 hover:bg-slate-600']">
                  - Потери
                </button>
              </div>
            </div>
