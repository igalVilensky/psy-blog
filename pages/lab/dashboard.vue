...
<template>
  <div class="min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-500 relative">
      ...
      <div class="bg-white dark:bg-slate-800/30 rounded-xl p-6 border border-slate-200 dark:border-slate-700/50 shadow-sm dark:shadow-none">
        ...
        <div class="h-64 flex items-end justify-between space-x-2">
          <div
            ...
            :key="index"
            class="flex-1 relative group h-full flex flex-col justify-end"
          ...
          >
            ...
            <div class="text-center mt-2">
              ...
              >
                {{ bar.absoluteValue }} {{ bar.absoluteValue === 1 ? 'действие' : (bar.absoluteValue > 1 && bar.absoluteValue < 5 ? 'действия' : 'действий') }}
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
      ...
      <!-- Recent Achievements -->
      <div v-if="achievements.length > 0" class="bg-white dark:bg-slate-800/30 rounded-xl p-6 border border-slate-200 dark:border-slate-700/50 shadow-sm dark:shadow-none">
    ...
    <div v-if="isExerciseModalOpen" class="relative z-50">
      <!-- Backdrop -->
      <div class="fixed inset-0 bg-black/25 dark:bg-black/40 backdrop-blur-sm transition-opacity" @click="closeExerciseModal"></div>
          ...
          >
            ...
            <div class="mt-6 space-y-6" v-if="selectedExercise">
              ...
              <div>
                ...
                >
                  <div
                    v-for="(session, index) in selectedExercise.history || []"
                    :key="index"
                  ...
                  <div
                    v-if="!selectedExercise.history?.length"
...
const progressData = computed(() => {
  ...
  // 1. Pattern Results
  patternResults.value.forEach(r => addInteraction(r.timestamp));
...
function calculateStreak() {
    ...
    const days = new Set(allResults.map(r => {
        ...
        return new Date((r.timestamp || r.createdAt).seconds * 1000).toDateString();
    }).filter(Boolean));
    return days.size;
}
...
const fetchUserData = async () => {
  ...
  try {
    ...
    const qPatterns = query(collection(db, `users/${authStore.user.uid}/patternDetectionResults`), orderBy("timestamp", "desc"));
