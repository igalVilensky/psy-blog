  ...
  <div class="min-h-screen bg-slate-50 dark:bg-slate-950 px-4 py-8 font-sans transition-colors duration-500">
    <div class="max-w-4xl mx-auto">
      ...
      <div v-if="gameState === 'start'" class="text-center space-y-8 animate-fade-in-up">
        ...
        <div class="bg-white dark:bg-slate-900/50 rounded-3xl p-8 border border-slate-200 dark:border-white/10 shadow-xl backdrop-blur-sm text-left">
          ...
          <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-4 mb-8">
            <div class="p-4 rounded-2xl bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-500/20">
              <div class="w-8 h-8 rounded-full bg-blue-500 text-white flex items-center justify-center font-bold mb-3 shadow">1</div>
              <p class="text-sm text-slate-700 dark:text-slate-300">Цвет фона подсказывает, какое правило сейчас активно</p>
            </div>
             <div class="p-4 rounded-2xl bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-500/20">
              <div class="w-8 h-8 rounded-full bg-indigo-500 text-white flex items-center justify-center font-bold mb-3 shadow">2</div>
              <p class="text-sm text-slate-700 dark:text-slate-300">Вы видите число, фигуру или цветное слово</p>
            </div>
             <div class="p-4 rounded-2xl bg-purple-50 dark:bg-purple-900/20 border border-purple-100 dark:border-purple-500/20">
              <div class="w-8 h-8 rounded-full bg-purple-500 text-white flex items-center justify-center font-bold mb-3 shadow">3</div>
             ...
             <div class="p-4 rounded-2xl bg-pink-50 dark:bg-pink-900/20 border border-pink-100 dark:border-pink-500/20">
              <div class="w-8 h-8 rounded-full bg-pink-500 text-white flex items-center justify-center font-bold mb-3 shadow">!</div>
              <p class="text-sm text-slate-700 dark:text-slate-300">Правила меняются каждые 3-6 раундов — будьте внимательны!</p>
            </div>
          </div>

          <h3 class="text-xl font-bold text-slate-900 dark:text-white mb-4">4 Правила (следите за цветом фона!)</h3>
          <div class="space-y-3 mb-8">
            <div class="flex items-center gap-4 p-3 rounded-xl bg-blue-100 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-700/50">
              ...
              <div class="flex-1">
                ...
                <span class="text-sm text-slate-600 dark:text-slate-400">Чётное (2, 4, 6, 8) → <b>ВЛЕВО</b> | Нечётное (1, 3, 5, 7, 9) → <b>ВПРАВО</b></span>
              </div>
            </div>
            <div class="flex items-center gap-4 p-3 rounded-xl bg-yellow-100 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-700/50">
              ...
              <div class="flex-1">
                ...
                <span class="text-sm text-slate-600 dark:text-slate-400">Меньше 5 (1, 2, 3, 4) → <b>ВЛЕВО</b> | 5 и больше (5, 6, 7, 8, 9) → <b>ВПРАВО</b></span>
              </div>
            </div>
             <div class="flex items-center gap-4 p-3 rounded-xl bg-green-100 dark:bg-green-900/30 border border-green-200 dark:border-green-700/50">
              ...
              <div class="flex-1">
                ...
                <span class="text-sm text-slate-600 dark:text-slate-400">Острые углы (◼️▲◆⭐⬡) → <b>ВЛЕВО</b> | Округлые (⭕🥚❤️🌙) → <b>ВПРАВО</b></span>
              </div>
            </div>
             <div class="flex items-center gap-4 p-3 rounded-xl bg-red-100 dark:bg-red-900/30 border border-red-200 dark:border-red-700/50">
              <div class="w-4 h-4 rounded-full bg-red-500 shadow flex-shrink-0 animate-pulse"></div>
              <div class="flex-1">
                <span class="font-bold text-red-800 dark:text-red-300 block">КРАСНЫЙ ФОН — Цвет текста (НЕ слово!)</span>
                <span class="text-sm text-slate-600 dark:text-slate-400">КРАСНЫЙ текст → <b>ВЛЕВО</b> | ЗЕЛЁНЫЙ или СИНИЙ текст → <b>ВПРАВО</b></span>
                <div class="text-xs text-red-600 dark:text-red-400 font-bold mt-1">⚠️ Игнорируйте значение слова, смотрите только на ЦВЕТ БУКВ!</div>
              </div>
            </div>
          </div>
          ...
          <div class="space-y-4">
            ...
            <div class="grid sm:grid-cols-3 gap-4">
               <button @click="selectLevel('easy')" class="difficulty-card group">
                  ...
                  <div class="absolute inset-0 border-2 border-transparent group-hover:border-emerald-500/50 rounded-xl transition-all"></div>
              </button>
               <button @click="selectLevel('medium')" class="difficulty-card group">
                  ...
                  <div class="absolute inset-0 border-2 border-transparent group-hover:border-yellow-500/50 rounded-xl transition-all"></div>
              </button>
               <button @click="selectLevel('hard')" class="difficulty-card group">
                  ...
                  <div class="absolute inset-0 border-2 border-transparent group-hover:border-red-500/50 rounded-xl transition-all"></div>
              </button>
            </div>
          </div>
        </div>
      ...
      <!-- Game Running State -->
      <div v-else-if="gameState === 'running'" class="flex flex-col h-[80vh] items-center justify-center relative rounded-3xl overflow-hidden transition-colors duration-500 border-4 border-slate-200 dark:border-transparent shadow-2xl" :class="currentRule.bgClass">
        
        <!-- Progress Bar -->
        <div class="absolute top-0 left-0 right-0 h-2 bg-black/10">
          <div class="h-full bg-white/50 transition-all duration-300" :style="{ width: `${(currentRound / totalRounds) * 100}%` }"></div>
        </div>

        <!-- Timer/Stats -->
        <div class="absolute top-6 left-6 right-6 flex justify-between text-black/60 dark:text-white/80 font-bold text-xl mix-blend-overlay">
          <span>Раунд {{ currentRound }} / {{ totalRounds }}</span>
          <span>Ошибки: {{ errors }}</span>
        </div>

        <!-- Stimulus -->
        <div class="relative z-10 transform transition-all duration-200" :key="currentStimulus.id" :class="{'scale-110': feedback === 'correct', 'scale-90 shake': feedback === 'wrong'}">
           <!-- Text Stimulus (Number/Word) -->
           <div v-if="currentStimulus.type === 'text'" class="text-7xl sm:text-9xl font-black drop-shadow-xl select-none" :style="{ color: currentStimulus.color }">
             {{ currentStimulus.value }}
           </div>
           <!-- Icon Stimulus -->
           <div v-else-if="currentStimulus.type === 'icon'" class="text-8xl sm:text-[10rem] drop-shadow-xl select-none animate-bounce-slight" :style="{ color: currentStimulus.color }">
             {{ currentStimulus.value }}
           </div>
        </div>
        ...
        <div v-if="feedback" class="absolute inset-0 flex items-center justify-center z-20 pointer-events-none">
            <i v-if="feedback === 'correct'" class="fas fa-check text-8xl text-white/80 drop-shadow-lg animate-scale-in"></i>
        ...
        <div class="absolute bottom-10 left-0 right-0 flex justify-between px-10 sm:px-20 opacity-80 z-30">
            <button @click.stop="handleInput('left')" class="text-center group active:scale-95 transition-transform p-4 rounded-xl hover:bg-white/10">
              <div class="text-4xl mb-2 group-hover:-translate-x-1 transition-transform">⬅️</div>
              <div class="font-bold uppercase tracking-widest text-sm bg-white/20 px-3 py-1 rounded-lg backdrop-blur shadow-sm">Влево</div>
              <div class="text-xs mt-1 font-medium bg-black/20 text-white px-2 py-1 rounded inline-block">{{ getLeftHint() }}</div>
            </button>
            <button @click.stop="handleInput('right')" class="text-center group active:scale-95 transition-transform p-4 rounded-xl hover:bg-white/10">
              <div class="text-4xl mb-2 group-hover:translate-x-1 transition-transform">➡️</div>
              <div class="font-bold uppercase tracking-widest text-sm bg-white/20 px-3 py-1 rounded-lg backdrop-blur shadow-sm">Вправо</div>
               <div class="text-xs mt-1 font-medium bg-black/20 text-white px-2 py-1 rounded inline-block">{{ getRightHint() }}</div>
            </button>
        </div>
      </div>
       ...
       <div v-else-if="gameState === 'finished'" class="text-center space-y-8 animate-fade-in-up">
          <div class="bg-white dark:bg-slate-900 rounded-3xl p-8 sm:p-12 border border-slate-200 dark:border-white/10 shadow-2xl max-w-2xl mx-auto">
            <div class="w-20 h-20 bg-gradient-to-br from-emerald-500 to-green-600 rounded-full flex items-center justify-center mx-auto mb-6 shadow-lg shadow-emerald-500/30">
            ...
            <div class="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <button @click="startGame" class="px-8 py-4 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-bold hover:shadow-lg hover:shadow-indigo-500/25 transition transform hover:-translate-y-1">
                Повторить
              </button>
            </div>
          </div>
