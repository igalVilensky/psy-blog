    ...
    <div class="relative p-6 lg:p-8 space-y-8">
            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <div class="flex items-center gap-2 mb-2">
                        ...
                        <span
                            class="text-xs font-semibold uppercase tracking-wide text-stone-500 dark:text-stone-400">Панель
                            Коуча</span>
                    </div>
                    ...
                    <p class="text-stone-600 dark:text-stone-400 mt-2 text-sm max-w-md">
                        Полный список ваших клиентов, их статус и прогресс
                    </p>
                </div>
        ...
        <div class="relative z-10 space-y-6">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <h2 class="text-xl font-bold text-stone-900 dark:text-white">
                    Список клиентов <span class="text-stone-400 dark:text-stone-500 font-normal text-base">({{
                ...
                <div class="relative">
                    <input v-model="searchQuery" type="text" placeholder="Поиск по имени или email..."
                        class="w-full sm:w-64 pl-9 pr-4 py-2 bg-white dark:bg-stone-900 border border-stone-300 dark:border-stone-700 text-sm focus:outline-none focus:border-stone-900 dark:focus:border-white transition-colors">
                    <i class="fas fa-search absolute left-3 top-1/2 -translate-y-1/2 text-stone-400 text-xs"></i>
                </div>
            </div>

            <div v-if="loading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div v-for="i in 6" :key="i" class="h-56 bg-stone-100 dark:bg-stone-900 animate-pulse">
                </div>
            </div>

            <div v-else-if="filteredClients.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <ClientCard v-for="client in filteredClients" :key="client.id" :client="client"
                    @click="navigateToClient(client.id)" />
            </div>

            <div v-else class="text-center py-20 border-2 border-dashed border-stone-200 dark:border-stone-800">
                <i class="fas fa-user-slash text-4xl text-stone-300 dark:text-stone-700 mb-4"></i>
                <h3 class="text-lg font-semibold text-stone-900 dark:text-white mb-2">Клиенты не найдены</h3>
                <p class="text-stone-500 dark:text-stone-400 text-sm">
                    Попробуйте изменить поисковый запрос или добавьте нового клиента
                </p>
            </div>
        </div>
        ...
        <div v-if="isInviteModalOpen" class="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div
                ...
                <div class="space-y-4">
                    <div
                        class="p-4 bg-stone-50 dark:bg-stone-800 border border-stone-200 dark:border-stone-700 relative">
                        <label
                        ...
                        <button @click="copyInviteLink"
                            ...
                            <i class="fas text-xs"
                                :class="copied ? 'fa-check text-emerald-600' : 'fa-copy text-stone-500'"></i>
                        </button>
                    </div>
